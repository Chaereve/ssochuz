...
const MEASURE = () => {
  ...
  /* mép TRONG của một khối = mép border trừ padding: đây là đường mà chữ, ô nhập
     và nút phải cùng nằm trên */
  const inner = (el) => el ? { left: el.getBoundingClientRect().left + pad(el, 'Left'), right: el.getBoundingClientRect().right - pad(el, 'Right') } : null;
  return {
...
(async () => {
  ...
  try {
    ...
    for (const width of WIDTHS) {
      ...
      assert.ok(m.docOverflow <= 1, width + 'px: trang tràn ngang ' + m.docOverflow + 'px');
    ...
    for (const width of [1440, 390]) {
      ...
      await p.waitForSelector('#shelfDialog:not([open])');
      ...
      await p.waitForSelector('#storyShelfDialog:not([open])');
